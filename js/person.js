...
Person.render = function ( config, personItem ) {
    ...
    component.load( "person.html", function() {
        ...
        theMovieDb.people.getById({ "id": personItem.id }, function( resp ){
            ...
            Person.renderFilmography(config, component, personItem.id);
...
Person.renderFilmography = function (config, component, personID ) {

    var filmographyElement = component.find("#filmography");

    theMovieDb.people.getMovieCredits({"id":personID}, function(resp){

        var credits = $.parseJSON(resp);

        // Known For..
        Person.renderKnownFor(config, component.find("#person-known_for"), credits.cast.slice(0,4) );
        ...
        $.map( credits.cast, function ( movie ) {
            ...
            var item = $(
                '<li class="timeline-inverted">'
                 ...
                 +'  </div>'
                +'</li>').appendTo(filmographyElement);
...
Person.renderKnownFor = function (config, knownForElement, movies ) {
